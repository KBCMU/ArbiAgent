...
class Market(BaseModel):
    """Prediction market data from DomeAPI (supports Polymarket and Kalshi)."""
    # Source platform
    source: str = "polymarket"  # "polymarket" or "kalshi"
    
    # Common identifiers (some optional depending on source)
    market_slug: Optional[str] = None  # Polymarket
    event_slug: Optional[str] = None  # Polymarket
    condition_id: Optional[str] = None  # Polymarket
    ticker: Optional[str] = None  # Kalshi
    ...
    side_a: Optional[MarketSide] = None
    side_b: Optional[MarketSide] = None
    winning_side: Optional[str] = None
    
    # Kalshi-specific pricing (cents)
